...
if ($_SESSION['user'] == 'student') {
    ...
    if (isset($_GET['token'])) {
        ...
        $sql = "SELECT * FROM `student` WHERE `s_token`='$token'";
        $qurey = mysqli_query($con, $sql);
        $rowcount = mysqli_num_rows($qurey);

        if ($rowcount == 1) {
            # code...
            $sql = "SELECT * FROM `student` WHERE `s_token`='$token' AND `s_active`= 'no'";
            $qurey = mysqli_query($con, $sql);
            $rowcount = mysqli_num_rows($qurey);
            if ($rowcount == 1) {
                # code...
                ...
                try {
                    //code...
                    $sql = "UPDATE `student` SET `s_active`='yes', `s_token`='' WHERE `s_token`='$token' AND `s_active`= 'no'";
